(function () {
  ...
  log('סביבה: פיתוח (DEV) - לוגים פעילים');
  ...
  var io = new IntersectionObserver(function (entries) {
    entries.forEach(function (e) {
      ...
      /* stagger sibling cards/steps */
      var siblings = el.parentElement ? el.parentElement.querySelectorAll('[data-animate]') : [];
  ...
  function setError(groupId, hasError) {
    ...
    if (!g) return;
    g.classList.toggle('error', hasError);
    var err = g.querySelector('.err-msg');
    if (err) err.setAttribute('aria-hidden', hasError ? 'false' : 'true');
  }
  ...
  form.addEventListener('submit', function (e) {
    e.preventDefault();
    if (!validate()) {
      var firstErr = form.querySelector('.form-group.error input, .form-group.error select');
    ...
    fetch('https://zelsnjtiahstjvoohvpb.supabase.co/rest/v1/rpc/submit_lead', {
      ...
      body: JSON.stringify({
        ...
        p_rooms: roomsSafe, p_price: price, p_notes: notes || '', p_language: 'he'
      })
    }).catch(function () {});
    ...
    fetch('https://api.web3forms.com/submit', {
      ...
      body: JSON.stringify({
        ...
        subject: 'פנייה חדשה - פסגות נכסים ערד',
        from_name: 'דף נחיתה פסגות נכסים',
        שם: name,
        טלפון: phone,
        מייל: email || '-',
        כתובת: address || '-',
    ...
    .then(function (data) {
      if (data.success) {
        ...
        window.dataLayer.push({
          event: 'generate_lead',
          event_category: 'form',
          event_label: 'arad_property_lead',
          rooms: roomsSafe,
          price: price
        });
      } else {
        btn.disabled = false;
        btn.textContent = 'שלח פנייה - נחזור אליך בהקדם';
      }
    })
    .catch(function () {
      btn.disabled = false;
